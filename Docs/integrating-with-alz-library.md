...
By default ALZ specifies deploying all the guardrail policies to the `platform` and `landingzones` management group and when the `Sync-ALZPolicyFromLibrary` command runs it will generate assignments which are scoped to these management groups.
