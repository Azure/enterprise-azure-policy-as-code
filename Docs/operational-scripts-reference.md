...
## Script `Update-PolicyAssignmentCsvParameterFile`

Updates an existing policy assignment parameter CSV in place by reconciling it with a current CSV generated by `Build-PolicyDocumentation`.

```ps1
Update-PolicyAssignmentCsvParameterFile -GeneratedCsvPath <String> -ParameterCsvPath <String> [-WhatIf] [-Confirm] [<CommonParameters>]
```

Rows are matched case-insensitively by `name` and `referencePath`. Generated metadata is refreshed, while existing columns ending in `Effect` or `Parameters` and target-only custom columns are preserved. Removed policies are deleted and new policies are added with generated defaults.
...
#### `-ParameterCsvPath <String>`

Path to the existing assignment parameter CSV to update in place. The file is not changed if validation or reconciliation fails.

#### `-WhatIf [<SwitchParameter>]`

Shows which parameter CSV would be updated without changing the file.

#### `-Confirm [<SwitchParameter>]`

Prompts for confirmation before replacing the parameter CSV.
